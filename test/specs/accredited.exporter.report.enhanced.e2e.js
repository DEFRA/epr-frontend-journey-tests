...
describe.skip('Accredited exporter report flow @accreditedExporter', () => {
  describe('accredited exporter with upload', () => {
    ...
    it('should navigate back correctly through the accredited exporter flow @accreditedExporterBackLinks @enhancedCheck', async () => {
    ...
    it('should navigate to delete confirmation from PRN summary and free PERNs pages @accreditedExporterDelete @enhancedCheck', async () => {
      await ReportsPage.selectActiveActionLink(1)
      await ReportDetailPage.useThisData()
    ...
    it('should save and come back later from PRN summary and free PERNs pages @accreditedExporterSave @enhancedCheck', async () => {
      await ReportsPage.selectActiveActionLink(1)
      await ReportDetailPage.useThisData()
    ...
    it('should complete the full accredited exporter report flow through to confirmation @accreditedExporterFullFlow @enhancedCheck', async () => {
      await ReportsPage.selectActiveActionLink(1)
      await ReportDetailPage.useThisData()
  ...
  describe('non-accredited exporter route guards', () => {
    ...
    it('should return 404 when non-accredited exporter tries to access PRN pages @accreditedExporterRouteGuard @enhancedCheck', async () => {
    ...
    it('should return 404 when registered-only exporter tries to access PRN pages @registeredOnlyExporterRegression @enhancedCheck', async () => {
      // Try to access prn-summary directly — should get 404
      await browser.url(
        `/organisations/${orgRefNo}/registrations/${registrationId}/reports/2026/monthly/1/submissions/1/prn-summary`
      )
