...
async function uploadAndNavigateToReports() {
  ...
  await EnhancedUploadSummaryLogPage.performUploadAndReturnToHomepage(filePath)

  await DashboardPage.selectTableLink(1, 1)
  await WasteRecordsPage.manageReportsLink()
}

describe.skip('Accredited reprocessor report flow @accreditedReprocessor', () => {
  describe('accredited reprocessor with upload', () => {
    ...
    it('should navigate back correctly through the accredited reprocessor flow @accreditedReprocessorBackLinks @enhancedCheck', async () => {
    ...
    it('should navigate to delete confirmation from tonnes recycled and PRN summary pages @accreditedReprocessorDelete @enhancedCheck', async () => {
      await ReportsPage.selectActiveActionLink(1)
      await ReportDetailPage.useThisData()
    ...
    it('should save and come back later from tonnes recycled page @accreditedReprocessorSave @enhancedCheck', async () => {
      await ReportsPage.selectActiveActionLink(1)
      await ReportDetailPage.useThisData()
    ...
    it('should complete the full accredited reprocessor report flow through to confirmation with submission and unsubmission via backend @accreditedReprocessorFullFlow @smoketest @enhancedCheck', async () => {
      await ReportsPage.selectActiveActionLink(1)
      await ReportDetailPage.useThisData()
  ...
  describe('non-accredited reprocessor route guard', () => {
    it('should return 404 when registered-only reprocessor tries to access PRN pages @registeredOnlyReprocessorRouteGuard @enhancedCheck', async () => {
      const organisationDetails = await createLinkedOrganisation([
        {
          material: 'Paper or board (R3)',
          wasteProcessingType: 'Reprocessor',
