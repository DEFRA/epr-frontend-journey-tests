...
describe('lint-types-tests-summary', () => {
  describe(filterTestFiles, () => {
    ...
    it('should include files under docker/mock/', () => {
      const result = filterTestFiles([
        'docker/mock/handlers.js',
        'docker/scripts/floci/init.sh'
      ])

      expect(result).toStrictEqual(['docker/mock/handlers.js'])
    })

    it('should exclude archived_form_tests', () => {
      const result = filterTestFiles([
        'test/archived_form_tests/old.js',
