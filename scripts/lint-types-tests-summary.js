...
const TEST_FILE_PATTERNS = [/^test\/.*\.js$/, /^docker\/mock\/.*\.js$/]
...
export const filterTestFiles = (paths) =>
  paths.filter(
    (p) =>
      !p.includes('archived_form_tests/') &&
      TEST_FILE_PATTERNS.some((re) => re.test(p))
  )
...
const changedFilesFromGit = () => {
  ...
  const out = execSync(
    `git diff --name-only origin/${baseRef}...HEAD -- test/ docker/mock/`
  ).toString()
